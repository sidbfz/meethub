...
import { createDemoAuthUser, DEMO_MODE } from '@/lib/demo-mode';
...
export default function ModeratorHostChat({ eventId, hostId, moderatorId }: ModeratorHostChatProps) {
  ...
  const loadMessages = useCallback(async () => {
    ...
    if (DEMO_MODE) {
      const host = mockUsers.find(item => item.id === hostId) || mockUsers[0];
      const moderator = mockUsers.find(item => item.id === moderatorId) || mockUsers[0];
      setMessages([
        {
          id: `${eventId}-mod-demo-1`,
          event_id: eventId,
          sender_id: moderatorId,
          receiver_id: hostId,
          message_text: 'Hi, I reviewed the event details and had one quick question before approval.',
          created_at: new Date(Date.now() - 1000 * 60 * 20).toISOString(),
          users: [{
            id: moderator.id,
            full_name: moderator.name,
            email: moderator.email,
            avatar_url: moderator.avatarUrl,
          }],
        },
        {
          id: `${eventId}-mod-demo-2`,
          event_id: eventId,
          sender_id: hostId,
          receiver_id: moderatorId,
          message_text: 'Sure, happy to clarify anything.',
  ...
  const handleSendMessage = async () => {
    ...
    if (DEMO_MODE) {
      const sender = mockUsers.find(item => item.id === user.id) || mockUsers[0];
      const demoMessage: Message = {
        id: `${eventId}-mod-demo-${Date.now()}`,
        event_id: eventId,
        sender_id: user.id,
