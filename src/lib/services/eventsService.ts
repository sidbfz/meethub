...
import { getDemoEventMessages, isDemoEventId, toAppEvent } from '@/lib/demo-transforms';
...
export const eventsService = {
  ...
  async joinEvent(eventId: string, userId: string): Promise<void> {
    ...
    if (DEMO_MODE) {
      ...
      console.log('Demo Mode: Simulated event join');
      return;
    }
  ...
  async leaveEvent(eventId: string, userId: string): Promise<void> {
    ...
    if (DEMO_MODE) {
      await new Promise(resolve => setTimeout(resolve, 300));
      console.log('Demo Mode: Simulated event leave');
      return;
    }
  ...
  async isParticipant(eventId: string, userId: string): Promise<boolean> {
    ...
    const isDemoEvent = isDemoEventId(eventId);
  ...
  async getEventMessages(eventId: string) {
    if (DEMO_MODE) {
      await new Promise(resolve => setTimeout(resolve, 300));
      return getDemoEventMessages(eventId);
    }
  ...
  async addHostAsParticipant(eventId: string, hostId: string): Promise<void> {
    ...
    if (DEMO_MODE) {
      console.log('Demo Mode: Simulated adding host as participant');
      return;
    }
  ...
  async updateEvent(eventId: string, updates: Partial<Event>): Promise<Event> {
    ...
    if (DEMO_MODE) {
      await new Promise(resolve => setTimeout(resolve, 500));
      const mockEvent = mockEvents.find(event => event.id === eventId) || mockEvents[0];
      return {
        ...toAppEvent(mockEvent, 'pending_approval'),
        ...updates,
        id: eventId,
  ...
  async cancelEvent(eventId: string): Promise<void> {
    ...
    if (DEMO_MODE) {
      await new Promise(resolve => setTimeout(resolve, 300));
      console.log('Demo Mode: Simulated event cancellation');
      return;
    }
  ...
  async deleteEvent(eventId: string): Promise<void> {
    ...
    if (DEMO_MODE) {
      await new Promise(resolve => setTimeout(resolve, 300));
      console.log('Demo Mode: Simulated event deletion');
      return;
    }
  ...
  async getParticipantCount(eventId: string): Promise<number> {
    if (DEMO_MODE) {
      return mockEvents.find(event => event.id === eventId)?.currentParticipants || 0;
    }
