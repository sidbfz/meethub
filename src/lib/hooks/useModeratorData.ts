...
import { toAppEvent } from '@/lib/demo-transforms';
...
export function useModeratorData() {
  ...
  const getDemoModeratorEvents = () => mockEvents.map((mockEvent): PendingEvent => {
    const status = mockEvent.approvalRequired
      ? 'pending_approval'
      : mockEvent.status === 'completed'
        ...
        : mockEvent.status === 'cancelled'
          ? 'cancelled'
          : 'approved';
    const event = toAppEvent(mockEvent, status);
    ...
    return {
      ...
      current_participants: mockEvent.currentParticipants,
      host: {
  ...
  const fetchEventsByStatus = async (status: string) => {
    if (DEMO_MODE) {
      return getDemoModeratorEvents().filter(event => event.status === status);
    }
  ...
  const fetchAllEvents = async () => {
    try {
      if (DEMO_MODE) {
        ...
        const eventsWithCounts = getDemoModeratorEvents();
    ...
    try {
      ...
      if (DEMO_MODE) {
        ...
        if (eventToApprove) {
          ...
          setApprovedEvents(prev => [{ ...eventToApprove, status: 'approved' }, ...prev]);
          setStats(prev => ({
            ...prev,
  ...
  const rejectEvent = async (eventId: string) => {
    ...
    try {
      ...
      if (DEMO_MODE) {
        await new Promise(resolve => setTimeout(resolve, 300));
        const eventToReject = pendingEvents.find(event => event.id === eventId);
        if (eventToReject) {
          setPendingEvents(prev => prev.filter(event => event.id !== eventId));
          setRejectedEvents(prev => [{ ...eventToReject, status: 'rejected' }, ...prev]);
          setStats(prev => ({
            ...prev,
            pending: Math.max(0, prev.pending - 1),
